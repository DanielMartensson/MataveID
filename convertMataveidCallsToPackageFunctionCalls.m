...
% Get the  Mataveid m files
mcFiles = struct2table(dir(fullfile("sourcecode","+" + package,"*.m")));
mcFiles.name = string(mcFiles.name);
mifles.folder = string(mcFiles.folder);

[~,mcFunctionList] = fileparts(mcFiles.name);

% Get the Mataveid example m file list
...
for i = 1:size(sourceCodeFiles,1)
  ...
  for j = 1:size(mcFiles,1)
    replacementExpression = package + "." + mcFunctionList(j) +"(";
