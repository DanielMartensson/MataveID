...
% Example 1: [sysd, K, L] = mi.ocid(r, uf, y, sampleTime, alpha, regularization, systemorder);
% Example 2: [sysd, K, L] = mi.ocid(r, uf, y, sampleTime, alpha);
...
function [sysd, K, L] = ocid(varargin)
  ...
  if(length(varargin) >= 5)
    ...
    error('Missing filtering integer parameter');
  end
  ...
  for j = 1:q
    ...
    for i = 1:2:2*p
      ...
      a = Y11(index);
      [ahat, ahorizon, ak, anoise] = mi.pf(a, ahatp, ak, ahorizon, anoise);
      ahatp = ahat;

      % Particle filtering for Y12
      b = Y12(index);
      [bhat, bhorizon, bk, bnoise] = mi.pf(b, bhatp, bk, bhorizon, bnoise);
      bhatp = bhat;

      % Particle filtering for Y21
      c = Y21(index);
      [chat, chorizon, ck, cnoise] = mi.pf(c, chatp, ck, chorizon, cnoise);
      chatp = chat;

      % Particle filtering for Y22
      d = Y22(index);
      [dhat, dhorizon, dk, dnoise] = mi.pf(d, dhatp, dk, dhorizon, dnoise);
      dhatp = dhat;
