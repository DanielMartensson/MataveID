...
% Example 1: [X_point, Y_point, amount_of_supports_for_class] = Mid.nlsvm(X, Y);
...
function [X_point, Y_point, amount_of_supports_for_class] = nlsvm(X, Y)
  ...
  % Print data
  print_data(X, Y, X_point, Y_point, amount_of_supports_for_class);
...
function print_data(X, Y, X_point, Y_point, amount_of_supports_for_class)
  % Get the amount of classes
  c = size(X, 1);

  % Clear figure and add grid
  clf
  grid on
  title('Data and boundaries', 'FontSize', 30);
  ...
  % Create labels
  legend(legend_labels, 'AutoUpdate', 'off');

  % Now print the boundaries
  for i = 1:c
    hold on
    L = amount_of_supports_for_class(i);
    plot(X_point(i, 1:L), Y_point(i, 1:L))
  end

end

function [X_point, Y_point, amount_of_supports_for_class] = place_out_supports_points(X, Y)
  % Get the amount of classes
  c = size(X, 1);
...
function check_points_support(X, Y, X_point, Y_point)
  ...
  for i = 1:c
    ...
    if(counted_points == length(X(i, :)))
      disp(sprintf('Checking for class %i - All points include in the boundary', i));
    else
      disp(sprintf('Checking for class %i - Not all points are included in the boundary', i));
    end
  end
end
